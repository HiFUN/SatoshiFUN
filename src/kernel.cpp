...
// select a block from the candidate blocks in vSortedByTimesHIFUNp, excluding
// already selected blocks in vSelectedBlocks, and with timesHIFUNp up to
// nSelectionIntervalStop.
static bool SelectBlockFromCandidates(
    vector<pair<int64, uint256> >& vSortedByTimesHIFUNp,
    ...
    const CBlockIndex** pindexSelected)
{
    ...
    *pindexSelected = (const CBlockIndex*) 0;
    BOOST_FOREACH(const PAIRTYPE(int64, uint256)& item, vSortedByTimesHIFUNp)
    {
...
bool ComputeNextStakeModifier(const CBlockIndex* pindexPrev, uint64& nStakeModifier, bool& fGeneratedStakeModifier)
{
    ...
    // Sort candidate blocks by timesHIFUNp
    vector<pair<int64, uint256> > vSortedByTimesHIFUNp;
    vSortedByTimesHIFUNp.reserve(64 * nModifierInterval / nStakeTargetSpacing);
    ...
    {
        vSortedByTimesHIFUNp.push_back(make_pair(pindex->GetBlockTime(), pindex->GetBlockHash()));
        pindex = pindex->pprev;
    }
    int nHeightFirstCandidate = pindex ? (pindex->nHeight + 1) : 0;
    reverse(vSortedByTimesHIFUNp.begin(), vSortedByTimesHIFUNp.end());
    sort(vSortedByTimesHIFUNp.begin(), vSortedByTimesHIFUNp.end());

    // Select 64 blocks from candidate blocks to generate stake modifier
    uint64 nStakeModifierNew = 0;
    int64 nSelectionIntervalStop = nSelectionIntervalStart;
    map<uint256, const CBlockIndex*> mapSelectedBlocks;
    for (int nRound=0; nRound<min(64, (int)vSortedByTimesHIFUNp.size()); nRound++)
    {
        // add an interval section to the current selection round
        nSelectionIntervalStop += GetStakeModifierSelectionIntervalSection(nRound);
        // select a block from the candidates of current round
        if (!SelectBlockFromCandidates(vSortedByTimesHIFUNp, mapSelectedBlocks, nSelectionIntervalStop, nStakeModifier, &pindex))
...
//       (v0.2) nBits (deprecated): encodes all past block timesHIFUNps
//   txPrev.block.nTime: prevent nodes from guessing a good timesHIFUNp to
...
{
    if (nTimeTx < txPrev.nTime)  // Transaction timesHIFUNp violation
    ...
    {
        printf("CheckStakeKernelHash() : using modifier 0x%016"PRI64x" at height=%d timesHIFUNp=%s for block from height=%d timesHIFUNp=%s\n",
