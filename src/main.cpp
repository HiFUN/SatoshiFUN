...
const string strMessageMagic = "SatoshiFUN Signed Message:\n";
...
{
	int64 nSubsidy = 31250 * COIN;

	if(nHeight > 0 && nHeight < 11)
	{        
        nSubsidy = 250000000 * COIN;	// 2.5billion premine * 10
	}
	else if(nHeight > 10 && nHeight < 200){nSubsidy = 0 * COIN;}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // ppcoin: check transaction timesHIFUNp
            if (txPrev.nTime > nTime)
                return DoS(100, error("ConnectInputs() : transaction timesHIFUNp earlier than input transaction"));
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    // This rule was originally applied all blocks whose timesHIFUNp was after March 15, 2012, 0:00 UTC.
    // Now that the whole chain is irreversibly beyond that time it is applied to all blocks except the
    // two in the chain that violate it. This prevents exploiting the issue against nodes in their
    // initial block download.
    bool fEnforceBIP30 = true; // Always active in SatoshiFUN
    bool fStrictPayToScriptHash = true; // Always active in SatoshiFUN
...
bool CTransaction::GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        if (nTime < txPrev.nTime)
            return false;  // Transaction timesHIFUNp violation
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    if (GetBlockTime() > GetAdjustedTime() + nMaxClockDrift)
        return error("CheckBlock() : block timesHIFUNp too far in the future");
    ...
    // Check coinbase timesHIFUNp
    // if (GetBlockTime() > (int64)vtx[0].nTime + nMaxClockDrift)
    //    return DoS(50, error("CheckBlock() : coinbase timesHIFUNp is too early"));

    // Check coinstake timesHIFUNp
    if (IsProofOfStake() && !CheckCoinStakeTimesHIFUNp(GetBlockTime(), (int64)vtx[1].nTime))
        return DoS(50, error("CheckBlock() : coinstake timesHIFUNp violation nTimeBlock=%"PRI64d" nTimeTx=%u", GetBlockTime(), vtx[1].nTime));
    ...
    BOOST_FOREACH(const CTransaction& tx, vtx)
    {
        ...
        if (GetBlockTime() < (int64)tx.nTime)
            return DoS(50, error("CheckBlock() : block timesHIFUNp earlier than transaction timesHIFUNp"));
    }
...
bool CBlock::AcceptBlock()
{
    ...
    // Check timesHIFUNp against prev
    if (GetBlockTime() <= pindexPrev->GetMedianTimePast() || GetBlockTime() + nMaxClockDrift < pindexPrev->GetBlockTime())
        return error("AcceptBlock() : block's timesHIFUNp is too early");
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SatoshiFUN", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    if (fProofOfStake)  // attempt to find a coinstake
    {
        ...
        if (nSearchTime > nLastCoinStakeSearchTime)
        {
            ...
            if (pwallet->CreateCoinStake(*pwallet, pblock->nBits, nSearchTime-nLastCoinStakeSearchTime, txCoinStake))
            {
                ...
                {   // make sure coinstake would meet timesHIFUNp protocol
                    // as it would be the same as the block timesHIFUNp
    ...
    int64 nFees = 0;
    {
        ...
        if (pblock->IsProofOfStake())
            pblock->nTime      = pblock->vtx[1].nTime; //same as coinstake timesHIFUNp
...
//                        strMintWarning = strMintMessage;
            ...
            if (pblock->GetBlockTime() >= (int64)pblock->vtx[0].nTime + nMaxClockDrift)
                break;  // need to update coinbase timesHIFUNp
        }
    }
