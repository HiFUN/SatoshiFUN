...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "SatoshiFUN " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
    ...
    loop
    {
        ...
        if (addrman.size()==0 && (GetTime() - nStart > 60) && !fTestNet)
        {
            ...
            for (unsigned int i = 0; i < ARRAYLEN(pnSeed); i++)
            {
                ...
                // it'll get a pile of addresses with newer timesHIFUNps.
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. SatoshiFUN is probably already running."), addrBind.ToString().c_str());
        else
