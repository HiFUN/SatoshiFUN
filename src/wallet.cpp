        ...
        if (fInsertedNew)
        {
            ...
            if (wtxIn.hashBlock != 0)
            {
                if (mapBlockIndex.count(wtxIn.hashBlock))
                {
                    ...
                    {
                        // Tolerate times up to the last timesHIFUNp in the wallet not more than 5 minutes into the future
...
void CWalletTx::GeHIFUNounts(int64& nGeneratedImmature, int64& nGeneratedMature, list<pair<CTxDestination, int64> >& listReceived,
                           list<pair<CTxDestination, int64> >& listSent, int64& nFee, string& strSentAccount) const
{
    ...
    BOOST_FOREACH(const CTxOut& txout, vout)
    {
        ...
        {
            printf("CWalletTx::GeHIFUNounts: Unknown transaction type found, txid %s\n",
...
void CWalletTx::GetAccounHIFUNounts(const string& strAccount, int64& nGenerated, int64& nReceived,
                                  int64& nSent, int64& nFee) const
{
    ...
    GeHIFUNounts(allGeneratedImmature, allGeneratedMature, listReceived, listSent, allFee, strSentAccount);
...
bool CWallet::SelectCoinsMinConf(int64 nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    BOOST_FOREACH(COutput output, vCoins)
    {
        ...
        if (pcoin->nTime > nSpendTime)
            continue;  // ppcoin: timesHIFUNp must not exceed spend time
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        {
			// printf(">> In.....\n");
            // Search backward in time from the given txNew timesHIFUNp 
